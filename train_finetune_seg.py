#!/usr/bin/env python3
"""
train_finetune_seg.py

Fine-tune a pre-trained TransformerLifter on 2D poses + segment priors only
by zeroing out the camera-intrinsics branch and freezing all other weights
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in pbar:
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                pred = model(x2d, torch.zeros_like(batch['k']).to(device), seg)
                lm   = mpjpe(pred, y3d)
        ...
        if rank == 0:
            ...
            if vm < best_val:
                ...
                torch.save({
                    ...
                    'best_val': best_val
                }, f"best_lifter_seg_{D_MODEL}_ddp_epoch{epoch}.pt")
                print(f"→ Saved best_lifter_seg_{D_MODEL}_ddp_epoch{epoch}.pt (MPJPE={best_val:.4f})")
