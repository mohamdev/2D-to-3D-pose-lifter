...
Load a .npz with keys 'R', 't', and 'joints_3d', then:
...
def gv_init():
    ...
    try:
        ...
        print("Cannot initialize Gepetto-viewer. Is it installed & running?", e)
        sys.exit(1)
...
def main(npz_path):
    ...
    R         = data['R']          # (3,3) or (P,3,3)
    t         = data['t']          # (3,) or (P,3)
    joints_3d = data['joints_3d']  # (F,J,3)

    # ensure list of cameras
    if R.ndim == 2:
        Rs = R[None, ...]
        ts = t.reshape(1,3)
    elif R.ndim == 3:
        ...
        print("Expecting R.shape=(3,3) or (P,3,3).")
        sys.exit(1)

    F, J, _ = joints_3d.shape
    P = Rs.shape[0]
    print(f"Loaded {P} camera(s) and {F} frames of skeleton.")
    ...
    for (a, b) in CONNECTIONS:
        ...
        name = f'world/line_{a}_{b}'
        viz.viewer.gui.addLine(name, p1, p2, BONE_COLOR)
    ...
    viz.viewer.gui.addLine('world/line_spine', mid_hip0, mid_shoulder0, BONE_COLOR)
