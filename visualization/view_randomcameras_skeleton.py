...
def sample_direction() -> np.ndarray:
    """
    Sample a random unit direction uniformly on the sphere.
    """
...
def main():
    ...
    parser.add_argument('--dist-max', type=float, default=2.5,
    ...
    # Compute body radius & min joint z
    coords = joints_3d.reshape(-1, 3)
    R_body = np.linalg.norm(coords, axis=1).max()
    min_z = coords[:, 2].min()
    min_xy = R_body * (args.dist_min / 1.5)

    # Build extrinsics with random directions and distances
    extrinsics = []
    while len(extrinsics) < args.num_cams:
        v = sample_direction()
        factor = random.uniform(args.dist_min, args.dist_max)
        cam_pos = v * (R_body * factor)
        # enforce z and XY-plane distance constraints
        if cam_pos[2] < min_z:
            continue
        if np.linalg.norm(cam_pos[:2]) < min_xy:
            continue
        R_w2c = look_at_R(cam_pos)
        t_w2c = -R_w2c.dot(cam_pos)
        extrinsics.append((R_w2c, t_w2c))

    print(f"Generated {len(extrinsics)} camera extrinsics with z >= {min_z:.3f}, XY >= {min_xy:.3f}, and distance factor in [{args.dist_min}, {args.dist_max}].")
    viz = gv_init()
