#!/usr/bin/env python3
"""
train_finetune_versatile.py

Fine-tune a pre-trained TransformerLifter on 2D poses + priors,
randomly masking camera-intrinsics and segment-lengths at each step
(“versatile” training), freezing all other weights except the transformer
encoder and regressor, with tqdm logging and per-mode validation.
"""
import os
import argparse
import glob
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in pbar:
            ...
            # randomly mask priors during training
            m = random.randint(0, 3)
            k_in   = batch['k'].to(device)                    if (m & 1)==0 else torch.zeros_like(batch['k']).to(device)
            seg_in = batch['seg'].to(device)                  if (m & 2)==0 else torch.zeros_like(batch['seg']).to(device)

            opt.zero_grad()
            with torch.cuda.amp.autocast(enabled=args.amp):
                pred = model(x2d, k_in, seg_in)
                lm   = mpjpe(pred, y3d)
        ...
        # — validation across all four modes —
        model.eval()
        modes = {
            'full':    lambda b: (b['k'].to(device),                           b['seg'].to(device)),
            'cam_only':lambda b: (b['k'].to(device),           torch.zeros_like(b['seg']).to(device)),
            'seg_only':lambda b: (torch.zeros_like(b['k']).to(device), b['seg'].to(device)),
            'none':    lambda b: (torch.zeros_like(b['k']).to(device), torch.zeros_like(b['seg']).to(device)),
        }
        for mode, fn in modes.items():
            val_metrics = []
            pbar_val = tqdm(val_loader, desc=f"[Val-{mode}] Epoch {epoch}", disable=(rank!=0), ncols=120)
            with torch.no_grad():
            ...
            if rank == 0:
                print(f"[Epoch {epoch}] Val-{mode} ➜ MPJPE={vm:.4f}, BoneErr={vb:.4f}, Reproj={vr:.4f}")
                if mode == 'full' and vm < best_val:
                    best_val = vm
                    ...
                    torch.save({
                        ...
                        'best_val': best_val
                    }, f"best_lifter_versatile_{D_MODEL}_ddp_epoch{epoch}.pt")
                    print(f"→ Saved best_lifter_versatile_{D_MODEL}_ddp_epoch{epoch}.pt (full MPJPE={best_val:.4f})")
