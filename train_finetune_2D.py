#!/usr/bin/env python3
"""
train_finetune_2D.py

Fine-tune a pre-trained TransformerLifter on 2D poses only
by zeroing out both the camera-intrinsics and segment-lengths branches
and freezing all other weights except the transformer encoder and regressor,
with tqdm logging.
"""
import os
import argparse
...
def main():
    ...
    # ZERO-OUT both camera- and segment-prior branches
    with torch.no_grad():
        tgt.k_embed .weight.zero_()
        tgt.k_embed .bias.zero_()
        tgt.k_token .zero_()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch in pbar:
            ...
            with torch.cuda.amp.autocast(enabled=args.amp):
                pred = model(
                    x2d,
                    torch.zeros_like(batch['k']).to(device),
                    torch.zeros_like(batch['seg']).to(device)
                )
                lm   = mpjpe(pred, y3d)
        ...
        if rank == 0:
            ...
            if vm < best_val:
                ...
                torch.save({
                    ...
                    'best_val': best_val
                }, f"best_lifter_2D_{D_MODEL}_ddp_epoch{epoch}.pt")
                print(f"→ Saved best_lifter_2D_{D_MODEL}_ddp_epoch{epoch}.pt (MPJPE={best_val:.4f})")
